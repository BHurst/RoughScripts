    ...
    void FixedUpdate()
    {
        ...
        if (moveInput != new Vector2())
        {
            ...
            if (player.movementState == MovementState.Sprinting)
                ...
                fullSpeed *= player.totalStats.CastmoveSpeed_Rate_MultiplyPercent.value;
        ...
        if ((speedMagnitude < playerStatIncreasedSpeed && player.movementState != MovementState.Sprinting) || (speedMagnitude < playerStatIncreasedSpeed * player.totalStats.SprintSpeed_Rate_AddPercent.value && player.movementState == MovementState.Sprinting))
        ...
        else if (!player.pushedBeyondMaxSpeed && (speedMagnitude > playerStatIncreasedSpeed * player.totalStats.SprintSpeed_Rate_AddPercent.value && player.movementState == MovementState.Sprinting))
        {
            Vector2 capped = Vector2.ClampMagnitude(new Vector2() { x = playerBody.velocity.x, y = playerBody.velocity.z }, playerStatIncreasedSpeed * player.totalStats.SprintSpeed_Rate_AddPercent.value);
            playerBody.velocity = new Vector3() { x = capped.x, y = playerBody.velocity.y, z = capped.y };
        }
        else if (!player.pushedBeyondMaxSpeed && (speedMagnitude > playerStatIncreasedSpeed * player.totalStats.CastmoveSpeed_Rate_MultiplyPercent.value && player.movementState == MovementState.Casting))
        {
            Vector2 capped = Vector2.ClampMagnitude(new Vector2() { x = playerBody.velocity.x, y = playerBody.velocity.z }, playerStatIncreasedSpeed * player.totalStats.CastmoveSpeed_Rate_MultiplyPercent.value);
            playerBody.velocity = new Vector3() { x = capped.x, y = playerBody.velocity.y, z = capped.y };
        }
