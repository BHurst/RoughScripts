...
public class WeaponWorldAbility : RootWorldAbility
{
    public Transform ownersWeapon;
    RaycastHit[] weaponHit;
    public float abilityDuration;
    public float activationStart;
    public float activationEnd;
    ...
    void Update()
    {
        ...
        if (stats.abilityBaseDuration <= 0 && stats.castOnExpiration.Count == 0)
            Terminate();
        if (stats.abilityBaseDuration <= 0 && stats.castOnExpiration.Count > 0)
        {
            //Call for expiration effect
            Terminate();
        }
    ...
    void LateUpdate()
    {
        if (stats.abilityTags.Contains(AbilityTags.AbilityTag.Attack))
        {
