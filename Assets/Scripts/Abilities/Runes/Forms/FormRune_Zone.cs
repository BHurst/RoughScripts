...
public class FormRune_Zone : FormRune
{
    ...
    {
        return string.Format("Creates an area at the target for {0} seconds, that deals {1} {2} damage every {3} second to all targets within {4}m.", unitStats.GetDuration(ability), DamageManager.TooltipAbilityDamage(unitStats, ability), ability.schoolRune.schoolRuneType, formInterval, unitStats.GetArea(ability));
    }
}